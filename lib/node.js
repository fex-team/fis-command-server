...
function checkDeps(root) {
    ...
    if (fis.util.exists(pkgPath)) {
        ...
        for (var i = 0, len = modules.length; i < len; i++) {
            ...
            try {
                var main = resolve.sync(moduleId, {basedir: root});
                require(main); //check
            } catch(e) {
                return false;
            }
    ...
    step(
            ...
            function checkServerScript() {
                ...
                if(!fis.util.exists(script)){
                    ...
                    if (!fis.util.exists(path.join(root, 'package.json'))) {
                        fis.util.copy(path.join(__dirname, 'node', 'package.json'), path.join(root, 'package.json'));
                    }
